...
// Schéma de validation dynamique basé sur le type
...
const usagePercentage = computed(() => {
  if (!props.promotion?.usage_limit) return 0
  return Math.min(100, (props.promotion.usage_count / props.promotion.usage_limit) * 100)
})

// Charger les données de la promotion
watch([() => props.promotion, open], ([promo, isOpen]) => {
  if (promo && isOpen) {
    ...
    // Charger buy_x_get_y si c'est le type
    if (promo.type === 'buy_x_get_y' && promo.metadata) {
      state.buy_quantity = (promo.metadata as any).buy_quantity || null
      state.get_quantity = (promo.metadata as any).get_quantity || null
    } else {
      state.buy_quantity = null
      state.get_quantity = null
    }
  }
}, { immediate: true })
...
watch(() => state.type, (newType) => {
  // Ajuster la valeur si on change de type
  if ((newType === 'percentage' || newType === 'buy_x_get_y') && state.value && state.value > 100) {
    state.value = 100
  }
  // Si on passe à fixed_amount, suggérer un montant par défaut
...
<template>
  <UModal
    ...
    :description="`Modification de ${promotion?.name || 'la promotion'}`"
    :ui="{ content: 'min-w-[60%]' }"
  >
    <template #body>
      <UForm
      ...
      >
        ...
        <div class="border-t pt-4">
          ...
          <div class="mb-4 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800">
            <div class="flex items-center justify-between mb-2">
              <span class="text-sm text-gray-600 dark:text-gray-400">Utilisation actuelle</span>
              <span class="text-sm font-semibold text-gray-900 dark:text-white">
                {{ promotion.usage_count }} / {{ promotion.usage_limit || '∞' }}
              </span>
            </div>
            <div v-if="promotion.usage_limit" class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div
                class="h-2 rounded-full transition-all"
                :class="usagePercentage >= 80 ? 'bg-orange-500' : 'bg-primary-500'"
                :style="{ width: `${usagePercentage}%` }"
              />
            </div>
            <p v-if="promotion.usage_limit" class="text-xs text-gray-500 mt-1">
        ...
        <div class="border-t pt-4">
          <UFormField name="is_active">
            ...
            <template #hint>
              <span class="text-xs text-gray-500">
                {{ state.is_active ? 'La promotion est actuellement active' : 'La promotion est désactivée' }}
              </span>
            </template>
          </UFormField>
        </div>
