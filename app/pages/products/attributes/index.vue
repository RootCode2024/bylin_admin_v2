    ...
    <template #body>
      ...
      <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-2 w-full lg:w-auto flex-wrap">
          <UInput v-model="localSearch" icon="i-lucide-search" placeholder="Rechercher un attribut..."
          ...
          <USelectMenu v-model="localType" :items="Object.entries(typeLabels).map(([v, l]) => ({ label: l, value: v }))"
            value-key="value" label-key="label" class="w-48" />

          <USelectMenu v-model="localFilterable"
            :items="Object.entries(filterableLabels).map(([v, l]) => ({ label: l, value: v }))" value-key="value"
            label-key="label" class="w-52" />

          <UButton v-if="hasActiveFilters" icon="i-lucide-filter-x" color="gray" variant="ghost" label="Effacer filtres"
            @click="handleReset" />
        </div>
      ...
      <div
        ...
        <UTable ref="table" v-model:row-selection="rowSelection" :data="attributes as any" :columns="columns"
          ...
          <template #empty-state>
            <div class="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p class="text-sm text-gray-500 mt-1" v-if="localSearch || localType !== 'all'">
                Essayez de modifier vos filtres.
              </p>
              <UButton v-if="localSearch || localType !== 'all'" label="Réinitialiser" variant="link" class="mt-2"
                @click="handleReset" />
            </div>
          </template>
        </UTable>
